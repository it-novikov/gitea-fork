...
var (
	...
	tplKCPIdentityRecovery templates.TplName = "user/auth/kcp_identity_recovery"
...
func ForgotPasswd(ctx *context.Context) {
	if kcpidentity.Enabled() {
		...
		ctx.Data["Title"] = "KYBa account recovery"
		ctx.Data["Phone"] = ctx.FormString("phone")
		ctx.HTML(http.StatusOK, tplKCPIdentityRecovery)
		return
	}
...
func kcpIdentityStartRecovery(ctx *context.Context) {
	prepareKCPIdentityPageData(ctx)
	phone := ctx.FormString("phone")
	ctx.Data["Phone"] = phone
	if phone == "" {
		ctx.Data["Err_Phone"] = true
		ctx.RenderWithErrDeprecated("Registered phone is required.", tplKCPIdentityRecovery, nil)
		return
	}
	client := kcpidentity.NewClient()
	resp, err := client.StartRecovery(ctx, kcpidentity.RecoveryStartRequest{
		Phone:      phone,
	...
	if err != nil {
		ctx.RenderWithErrDeprecated("KYBa Identity Service did not start recovery: "+err.Error(), tplKCPIdentityRecovery, nil)
		return
	}
	ctx.Data["KCPIdentityRecoverySent"] = true
	ctx.Data["MaskedTarget"] = resp.MaskedTarget
	ctx.HTML(http.StatusOK, tplKCPIdentityRecovery)
}
