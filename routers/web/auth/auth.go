...
import (
	...
	kcpidentity_service "code.gitea.io/gitea/services/kcp/identity"
...
const (
	...
	tplKCPIdentitySignIn    templates.TplName = "user/auth/kcp_identity_signin"    // for KYBa Identity sign in
	tplKCPIdentityChallenge templates.TplName = "user/auth/kcp_identity_challenge" // for KYBa Identity challenge
	tplKCPIdentitySignUp    templates.TplName = "user/auth/kcp_identity_signup"    // for KYBa Identity registration
...
func prepareKCPIdentityPageData(ctx *context.Context) {
	cfg := kcpidentity.LoadSettings()
	ctx.Data["KCPIdentityEnabled"] = cfg.Enabled
	ctx.Data["KCPIdentityMock"] = cfg.Mock
	ctx.Data["KCPIdentityBaseURLConfigured"] = cfg.BaseURL != ""
	ctx.Data["KCPIdentitySignInLink"] = setting.AppSubURL + "/user/login"
	ctx.Data["KCPIdentityChallengeLink"] = setting.AppSubURL + "/user/identity/challenge"
	ctx.Data["KCPIdentitySignUpLink"] = setting.AppSubURL + "/user/sign_up"
	ctx.Data["KCPIdentityRecoveryLink"] = setting.AppSubURL + "/user/forgot_password"
...
func SignIn(ctx *context.Context) {
	...
	if kcpidentity.Enabled() {
		ctx.Data["Title"] = "KYBa Identity sign in"
		ctx.Data["PageIsSignIn"] = true
		ctx.Data["PageIsLogin"] = true
		ctx.Data["Phone"] = ctx.FormString("phone")
		prepareKCPIdentityPageData(ctx)
		ctx.HTML(http.StatusOK, tplKCPIdentitySignIn)
		return
	}
...
func kcpIdentityStartChallenge(ctx *context.Context) {
	...
	delivery := strings.TrimSpace(ctx.FormString("delivery"))
	if delivery == "" {
		delivery = "sms"
	}
	if phone == "" {
		ctx.Data["Err_Phone"] = true
		ctx.RenderWithErrDeprecated("Registered phone is required.", tplKCPIdentitySignIn, nil)
		return
	}
	client := kcpidentity.NewClient()
	resp, err := client.StartChallenge(ctx, kcpidentity.ChallengeStartRequest{
		Phone:      phone,
		Delivery:   delivery,
		RedirectTo: ctx.FormString("redirect_to"),
...
func IdentityChallenge(ctx *context.Context) {
	if !kcpidentity.Enabled() {
		ctx.NotFound(nil)
		return
	}
	...
	ctx.Data["MaskedTarget"] = masked
	ctx.HTML(http.StatusOK, tplKCPIdentityChallenge)
}

func IdentityChallengePost(ctx *context.Context) {
	if !kcpidentity.Enabled() {
		ctx.NotFound(nil)
		return
	}
	...
	if code == "" {
		...
		ctx.RenderWithErrDeprecated("Challenge code is required.", tplKCPIdentityChallenge, nil)
		return
	}
	client := kcpidentity.NewClient()
...
func SignUp(ctx *context.Context) {
	if kcpidentity.Enabled() {
		prepareKCPIdentityPageData(ctx)
		ctx.Data["Title"] = "Create KYBa account"
		ctx.Data["DisplayName"] = ctx.FormString("display_name")
		ctx.Data["Phone"] = ctx.FormString("phone")
		ctx.Data["InvitationCode"] = ctx.FormString("invitation_code")
		ctx.HTML(http.StatusOK, tplKCPIdentitySignUp)
		return
	}
...
func kcpIdentityRegister(ctx *context.Context) {
	prepareKCPIdentityPageData(ctx)
	displayName := strings.TrimSpace(ctx.FormString("display_name"))
	phone := strings.TrimSpace(ctx.FormString("phone"))
	invitationCode := strings.TrimSpace(ctx.FormString("invitation_code"))
	ctx.Data["DisplayName"] = displayName
	ctx.Data["Phone"] = phone
	ctx.Data["InvitationCode"] = invitationCode
	if displayName == "" || phone == "" || invitationCode == "" {
		ctx.RenderWithErrDeprecated("Display name, phone and invitation code are required.", tplKCPIdentitySignUp, nil)
		return
	}
	client := kcpidentity.NewClient()
